...
public class PacketRegistry {
        // public static final ReportType REPORT_CANNOT_CORRECT_TROVE_MAP = new
        // ReportType("Unable to correct no entry value.");
        //
        // public static final ReportType REPORT_INSUFFICIENT_SERVER_PACKETS = new
        // ReportType("Too few server packets detected: %s");
        // public static final ReportType REPORT_INSUFFICIENT_CLIENT_PACKETS = new
        // ReportType("Too few client packets detected: %s");
        ...
        // Packet IDs to classes, grouped by whether or not they're vanilla or
        // custom defined
        ...
        public static Map<Class, Integer> getPacketToID() {
                ...
                if (packetToID == null) {
                        try {
                                final Field packetsField = getPacketRegistry().getFieldByType("packetsField", Map.class);
                        ...
                        customIdToPacket = InverseMaps.inverseMultimap(packetToID, new Predicate<Map.Entry<Class, Integer>>() {
                                @Override
                                public boolean apply(@Nullable final Entry<Class, Integer> entry) {
                                        return !MinecraftReflection.isMinecraftClass(entry.getKey());
                                }
                        });

                        // And the vanilla pack - here we assume a unique ID to class
                        // mapping
        ...
        private static Map<Class, Integer> getSpigotWrapper() throws IllegalAccessException {
                ...
                final FuzzyClassContract mapLike = FuzzyClassContract.newBuilder()
                        .method(FuzzyMethodContract.newBuilder().nameExact("size").returnTypeExact(int.class))
                        .method(FuzzyMethodContract.newBuilder().nameExact("put").parameterCount(2))
                        .method(FuzzyMethodContract.newBuilder().nameExact("get").parameterCount(1)).build();

                final Field packetsField = getPacketRegistry().getField(FuzzyFieldContract.newBuilder().typeMatches(mapLike).build());
                ...
                try {
                        ...
                        final Integer value = (Integer) FieldUtils.readField(field, troveMap, true);
                        ...
                        DebugLog.addException("Unable to get no entry value", e);
                }
        ...
        /**
         * Retrieve the cached fuzzy reflection instance allowing access to the
         * packet registry.
         *
         ...
         * @throws FieldAccessException If we're unable to retrieve the server
         * packet data from Minecraft.
         */
         ...
         * @throws FieldAccessException If we're unable to retrieve the client
         * packet data from Minecraft.
         */
        ...
        private static void initializeSets() throws FieldAccessException {
                if (serverPacketsRef == null || clientPacketsRef == null) {
                        final List<Field> sets = getPacketRegistry().getFieldListByType(Set.class);
                        if (sets.size() > 1) {
                                ...
                                if (serverPackets.size() < MIN_SERVER_PACKETS) {
                                        ...
                                        // Report.newBuilder(REPORT_INSUFFICIENT_SERVER_PACKETS).messageParam(serverPackets.size()));
                                        DebugLog.addInfo("Too few server packets detected :" + serverPackets.size());
                                }
                                if (clientPackets.size() < MIN_CLIENT_PACKETS) {
                                        // ProtocolLibrary.getErrorReporter().reportWarning(PacketRegistry.class,
                                        // Report.newBuilder(REPORT_INSUFFICIENT_CLIENT_PACKETS).messageParam(clientPackets.size()));
                                        DebugLog.addInfo("Too few client packets detected :" + serverPackets.size());
                                }
